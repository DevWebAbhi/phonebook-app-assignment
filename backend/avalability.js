...
avaliabilityRouter.post("/post", async (req, res) => {
    try {
        const { id, day, date, startTime, endTime, u_startTime, u_endTime } = req.body;
        ...
        console.log(id, day, date, startTime24, endTime24, u_startTime24, u_endTime24);

        const post = await executeQuery({
            query: "CALL phonebook.CreateContactAvailability(?, ?, ?, ?, ?, ?, ?)",
            values: [id, day, date, startTime24, endTime24, u_startTime24, u_endTime24]  // Use 24-hour times
        });
